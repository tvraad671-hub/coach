...
struct TodayMatchesLeagueDescriptor: Hashable {
    let id: Int
    let name: String
    let priority: Int
}

enum TodayMatchesLeagueConfig {
    static let allowedLeagues: [TodayMatchesLeagueDescriptor] = [
    ...
    static let allowedLeagueNameByID: [Int: String] = Dictionary(
        uniqueKeysWithValues: allowedLeagues.map { ($0.id, $0.name) }
    )
